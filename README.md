...
`ExportOptions.plist` настроен на прямую загрузку в App Store Connect. Расширению трансляции (запись онлайн-встреч) нужен App Group `group.kz.adv.meetings`, привязанный к App ID `kz.adv.meetings` и `kz.adv.meetings.broadcast`: автоподпись через ключ API создать группу не может — один раз зарегистрируйте её в Apple Developer → Identifiers → App Groups и включите в обоих App ID (capability App Groups на них уже включена: `scripts/asc.ts app-groups`). Пока группы нет, собирайте по `project-lite.yml` (без расширения; пункт «Записать онлайн-встречу» в приложении скрыт автоматически). Статус обработки, группы и крэш-логи из TestFlight: `pnpm --filter @adv/server exec tsx scripts/asc.ts builds | group | testers | crashes`. Внутренних тестировщиков (участников команды) добавляют в группу в App Store Connect; внешних — по email через `asc.ts add`, первая внешняя сборка проходит Beta App Review.
